...
#block(
  ...
  radius: 4pt,
  width: 100%,
  raw("wait <- faithful$waiting
n <- length(wait)", lang: "r")
)
...
#block(
  ...
  raw("hist(wait,
     ...
     xlab = \"Minutes\",
     col = \"steelblue\")", lang: "r")
)#align(center, image("figure/example1/unnamed-chunk-3-1.png", width: 80%))
...
#block(
  ...
  radius: 4pt,
  width: 100%,
  raw("p6 <- dbinom(6, size = 8, prob = 0.65)", lang: "r")
)
...
#block(
  ...
  radius: 4pt,
  width: 100%,
  raw("ci <- t.test(wait)$conf.int   # results='hide': compute now, report inline", lang: "r")
)

which evaluates to (69.274, 72.52) minutes. We are 95% confident the
...
#block(
  ...
  radius: 4pt,
  width: 100%,
  raw("fit <- lm(waiting ~ eruptions, data = faithful)
slope <- coef(fit)[2]
...
abline(fit, lwd = 2)", lang: "r")
)#align(center, image("figure/example1/unnamed-chunk-6-1.png", width: 80%))

Each additional minute of eruption predicts a 10.73-minute longer wait.
The slope is overwhelmingly significant ($p = 8.13 times 10^(-100)$ — tiny inline values
